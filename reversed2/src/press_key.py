...
# App: gta5vn-chat-go-premium v1.0.0 (CypherSoft)
...
def press_key(key: str, seed: Optional[int] = None, fast_mode: bool = False):
    if seed is not None:
        random.seed(seed)

    if fast_mode:
        hold = random.uniform(0.01, 0.02)      # giữ phím 10-20ms (nhanh)
        pre = random.uniform(0.005, 0.015)      # trễ trước 5-15ms
    else:
        hold = random.uniform(0.077, 0.106)     # giữ phím ~80-106ms
        pre = random.uniform(0.082, 0.131)      # trễ trước ~82-131ms
