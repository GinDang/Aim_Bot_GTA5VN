...
class MainPage(QWidget):
    def __init__(self, tool_bot, count_device_remain, expiration_time):
        ...
        # --- Chcdn ceda sd5 game ---
        self.window_combo = QComboBox()
        self.window_combo.setStyleSheet(
            "background-color: #2c2f33; color: white; padding: 8px; border-radius: 8px;"
        )
        self.window_combo.setPlaceholderText("Chcdn ceda sd5 game")
        self.refresh_windows()

        # --- Nft baft/taft + Cafi tabt ---
        main_button_layout = QHBoxLayout()
        self.start_button = ModernButton("B\u1eaft \u0111\u1ea7u", "#33383D")
        self.pause_button = ModernButton("T\u1ea1m d\u1eebng", "#33383D")
        self.settings_button = ModernButton("C\u00e0i \u0111\u1eb7t", "#33383D")
        main_button_layout.addWidget(self.start_button)
        main_button_layout.addWidget(self.pause_button)
        main_button_layout.addWidget(self.settings_button)
    ...
    def refresh_windows(self):
        """Load danh sách cửa sổ đang mở vào combobox."""
        self.window_combo.clear()
        self.window_combo.addItem("Chon Cua So Game", None)
        if not win32gui:
            return
    ...
    def start_tool(self):
        if not self.is_tool:
            try:
                # Lấy PID từ cửa sổ game đang chọn rồi set vào utils.PID
                hwnd = self.window_combo.currentData()
                if hwnd:
                    pid = ctypes.c_ulong()
                    ctypes.windll.user32.GetWindowThreadProcessId(int(hwnd), ctypes.byref(pid))
                    utils.set_pid(pid.value)
                    print("DEBUG Selected HWND=", hwnd, "PID=", pid.value)
                else:
                    print("DEBUG No window selected; PID stays:", getattr(utils, "PID", None))
